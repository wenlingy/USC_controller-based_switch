...
"""
2017 Spring EE_555 Final Project --- Part 2 Advanced Topology : Two-Switch-Network
...
"""
'''
Part2 Advanced Topology-- Two-Switch-Network

The controller my_router3 stored as my_router3.py

This implements a controller function on a 2-switch-forward-network
...
TABLE1 = ['00:00:00:00:00:01', '00:00:00:00:00:02', '20:00:00:00:00:00']
TABLE2 = ['00:00:00:00:00:03', '10:00:00:00:00:00']
class Tutorial (object):
    ...
    def __init__ (self, connection):
        ...
        self.routing_table1 = { '10.0.1.2': ['10.0.1.2', 
                                             's1-eth1', 
                                             '10.0.1.1', 
                                             1, 
                                             '00:00:00:00:00:01'],
        ...
        self.routing_table2 = { '10.0.1.0/24': ['10.0.1.1', 
                                             's2-eth2', 
                                             '10.0.1.1', 
                                             2, 
                                             '10:00:00:00:00:00'],
    ...
    def find_subnet(self, dst_ip, table_num):
        if table_num == 1:
            for subnet in self.routing_table1.keys():
                if dst_ip.inNetwork(subnet):
                    return subnet
        elif table_num == 2:
            for subnet in self.routing_table2.keys():
                if dst_ip.inNetwork(subnet):
                    return subnet
        return None
        ...
        # Handle IP type packet
            ...
            if ip_packet.protocol == pkt.ipv4.ICMP_PROTOCOL:
                ...
                if icmp_packet.type == pkt.TYPE_ECHO_REQUEST:
                    log.debug("ICMP request received + table_num = %d" % table_num)
            ...
            # Handle normal IP type packet
                ...
                if subnet != None:
                    ...
                    if table_num == 1:
                        packet.dst = adr.EthAddr(self.routing_table1[subnet][4])
                        self.send_EtherNet_packet(packet, self.routing_table1[subnet][3]) 
                    elif table_num == 2:
                        packet.dst = adr.EthAddr(self.routing_table2[subnet][4])
                        self.send_EtherNet_packet(packet, self.routing_table2[subnet][3]) 
